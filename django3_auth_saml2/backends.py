...
class SAML2AttrEmailUserBackend(RemoteUserBackend):
    """
    Uses the email username as the Username. Use the SAML2 attributes to configure
    the first name, and last name values. This presumes that the
    SAML2 SSO system has been setup to provide the attributes:
    ...
    The User name will be set to the users email username in lower-case.
    """
    def authenticate(self, request: WSGIRequest, remote_user: str) -> User:
        """
        This method must use the SAML2 attributes to formulate the User name
